...
def get_topics(vectorized_corpus, model):
    ...
    topics = [
        max(model[doc], key=itemgetter(1))[0] if len(doc) > 0 else 'none'
        for doc in vectorized_corpus
    ]

    return topics

gensim_lda = pickle.load(open('path\\to\\models\\topics\\LsiModel.pkl', 'rb'))
...
raw_doc_reader = RawCorpusReader('path\\to\\raw_corpus\\basic_corpus', fileids=fileids, encoding=None, cat_pattern=cat_pattern)
...
def createResultCorpus(topics, raw_docs, results_dir="/results/LSA_topic_separated_docs"):
...
createResultCorpus(topics, raw_docs, results_dir="path\\to\\App\\results\\LSA_topic_separated_docs")
